  ...
  return (
    <>
      ...
      <motion.button
        ...
        className={`fixed bottom-6 right-6 w-14 h-14 rounded-full bg-gradient-to-r from-brand-primary to-brand-primary-dark text-white shadow-xl shadow-brand-primary-light/30 flex items-center justify-center z-40 ${isOpen ? 'hidden' : 'flex'}`}
      >
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed inset-0 md:inset-auto md:bottom-6 md:right-6 md:w-[400px] md:h-[650px] md:max-h-[85vh] bg-white dark:bg-slate-900 md:rounded-2xl shadow-2xl border-0 md:border border-slate-100 dark:border-slate-800 flex flex-col z-50 overflow-hidden"
            >
              <div className="bg-gradient-to-r from-brand-primary to-brand-primary-dark p-4 text-white flex items-center justify-between shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-brand-primary-light text-xs flex items-center gap-1">
                      <span className="w-2 h-2 rounded-full bg-brand-primary-light animate-pulse" /> Online
                    </p>
                  </div>
                </div>
                <button 
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 dark:bg-slate-900/50">
                {messages.map((msg) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <span className={`text-[10px] mt-1 block ${msg.role === 'user' ? 'text-brand-primary-light' : 'text-slate-400 dark:text-slate-500'}`}>
                ...
                {isTyping && (
                  ...
                  >
                    <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl rounded-tl-sm shadow-sm border border-slate-100 dark:border-slate-700 flex items-center gap-1">
              ...
              {messages.length < 3 && !isTyping && (
                <div className="px-4 pb-2 bg-slate-50 dark:bg-slate-900/50 flex gap-2 overflow-x-auto no-scrollbar shrink-0">
                  {quickReplies.map((reply, idx) => (
                    <button
                      key={idx}
                      onClick={() => handleSend(reply)}
                      className="whitespace-nowrap px-3 py-1.5 bg-brand-primary-light/20 dark:bg-brand-primary/10 text-brand-primary dark:text-brand-primary-light border border-brand-primary-light/50 dark:border-brand-primary/30 rounded-full text-xs font-medium hover:bg-brand-primary-light/40 dark:hover:bg-brand-primary/20 transition-colors"
                    >
                      {reply}
                    </button>
                  ))}
              ...
              <div className="p-4 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shrink-0">
                ...
                >
                  <input
                    ...
                    className="flex-1 bg-slate-100 dark:bg-slate-800 border-transparent focus:bg-white dark:focus:bg-slate-900 focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20 dark:focus:ring-brand-primary-dark/30 rounded-full px-4 py-2.5 text-sm transition-all outline-none text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500"
                  />
                  <button
                    type="submit"
                    disabled={!input.trim() || isTyping}
                    className="w-10 h-10 rounded-full bg-brand-primary text-white flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed hover:bg-brand-primary-dark transition-colors shrink-0"
                  >
